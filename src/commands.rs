    ...
    pub fn show_help(args: Vec<String>) -> Result<(), String> {
        ...
        if let Some(cmd_name) = args.get(0) {
            if let Some(cmd) = command_manager.get_command(cmd_name) {
                println!("{}: {}", 
                    colorize(&cmd.name, Color::CYAN),
                    colorize(&cmd.description, Color::GREEN));
                println!("Usage: {}", colorize(&cmd.usage, Color::YELLOW));
            } else {
                println!("{}", colorize(&format!("Command '{}' not found", cmd_name), Color::RED));
            }
        } else {
            println!("{}", colorize("Available commands:", Color::CYAN));
            for cmd in command_manager.commands.values() {
                println!("{}: {}", 
                    colorize(&cmd.name, Color::YELLOW),
                    colorize(&cmd.description, Color::GREEN));
            }
        }
        Ok(())
